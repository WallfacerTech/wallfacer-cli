...
`reorder` writes one parent's child order in a single atomic request. Pages and playbooks share one ordering under a parent, so the list is mixed and must be that parent's complete set of children; a list that omits, repeats, or imports a sibling is rejected before anything is written. `move` a playbook and only its parent and position change: no draft save, no publish, no trigger change, and no task. `--position` on `create`, on `move` and on `update` inserts rather than overwrites: later siblings shift down, the parent renormalizes to dense positions, and a value past the end appends, so the entry lands where the server puts it and the returned record carries the position it landed on. Leave it off and the entry goes to the end of its parent.
